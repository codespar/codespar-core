/**
 * @codespar/camel basic tests for 0.2.0.
 */

import { describe, it, expect } from "vitest";
import type { Session, Tool, ToolResult } from "@codespar/sdk";
import { toCamelTool, getTools, handleToolCall } from "../index.js";
...
describe("@codespar/camel", () => {
  it("toCamelTool wraps tool in function envelope with callable", () => {
    const session = fakeSession([makeTool()]);
    const fn = toCamelTool(makeTool(), session);
  ...
  it("getTools returns array of CAMEL tools", async () => {
