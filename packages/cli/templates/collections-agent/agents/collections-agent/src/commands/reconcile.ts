 ...
 * executions without an outcome, receivables the rail never saw, paid
 * charges the bundle does not hold. Read-only on the rail: an `executing`
 * execution is closed only from what the rail says, and nothing is issued.
 */
...
try {
  ...
  for (const e of s.engine.list()) {
    if (e.state === "executing") {
      ...
      findings.push({ execution_id: e.id, kind: closed.state === "executing" ? (closed.reason === "awaiting_settlement" ? "awaiting_payer" : "still_uncertain") : "closed_from_rail", detail: `executing -> ${closed.state}${closed.detail ? ` (${closed.detail})` : ""}` });
      continue;
    }
    for (const o of e.outcomes) {
      if (o.receipt_id && !existsSync(join(runsDir(), e.run_id, "receipts", `${o.receipt_id}.json`))) {
        findings.push({ execution_id: e.id, kind: "record_missing_locally", detail: `paid charge ${o.receipt_id} is not in runs/${e.run_id}/receipts` });
      }
    }
