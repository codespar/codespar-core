/**
 * @codespar/hermes basic tests for 0.4.0.
 */

import { describe, it, expect } from "vitest";
import type { Session, Tool, ToolResult } from "@codespar/sdk";
import { toHermesTool, getTools, handleToolCall } from "../index.js";
...
describe("@codespar/hermes", () => {
  it("toHermesTool creates tool with correct MCP-style shape", () => {
    const session = fakeSession([makeTool()]);
    const tool = toHermesTool(makeTool(), session);
  ...
  it("tool.call routes through session.execute", async () => {
    const session = fakeSession([makeTool()]);
    const tool = toHermesTool(makeTool(), session);
    const result = await tool.call({ amount: 50 });
