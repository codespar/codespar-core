#!/usr/bin/env bash
#
# Boleto-expiry fiscal remediation — boots @copilotkit/aimock as a stand-in Anthropic
...
# `mocks` map, posted on session create. That only works against a
...
if command -v docker >/dev/null 2>&1; then
  ...
  CONTAINER_NAME="codespar-example-boleto-expiry-fiscal-remediation-$$"
  RUNTIME_LOG="$SKELETON_DIR/.runtime.log"
  ...
  # four CI jobs stayed unexplained for three days (core#135). The runtime's
  # own startup errors name the cause precisely, so the container is kept
  # until cleanup_docker removes it explicitly.
  ...
  DEMO_API_TOKEN="${CODESPAR_API_KEY:-demo}"
