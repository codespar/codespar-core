  ...
  it("computes 6x, creates the installment payment, and issues the NF-e through the real chat-loop", async () => {
    ...
    const r3 = await session.send(
      [
        ...
        "lugares — 6x no cartao'). Then issue an NF-e via",
        "nuvem-fiscal__create_nfe with ambiente: 2 (homologação),",
        "natureza_operacao: 'Venda de mercadoria', a single item",
        "(codigo SOFA-3L-001, descricao 'Sofa 3 lugares', ncm 94013000,",
        "cfop 5102, unidade UN, quantidade 1, valor_unitario 4800,",
        "valor_total 4800), pagamento { forma_pagamento: '03', valor:",
        "4800, parcelas: 6 }, and dummy emitente / destinatario blocks",
        "(the demo handler ignores their content). Finally send a",
        "confirmation message via z-api__send_text to +5511987654321.",
      ].join(" "),
    );
