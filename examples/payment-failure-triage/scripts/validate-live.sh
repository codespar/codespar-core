#!/usr/bin/env bash
#
# Live LLM smoke — same two triage scenarios as validate.sh, but the runtime
...
# Claude actually reads each codespar_pay decline + category and decides the
# remediation.
#
...
system-prompt issues that change whether the agent triages correctly). No
...
if [ -n "${CODESPAR_BASE_URL:-}" ]; then
  ...
  echo "validate-live.sh:        or the live smoke will not triage correctly."
...
if command -v docker >/dev/null 2>&1; then
  CONTAINER_NAME="codespar-example-payment-failure-triage-live-$$"
  RUNTIME_LOG="$SKELETON_DIR/.runtime.log"
