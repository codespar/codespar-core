 ...
 * @codespar/types/testing. This example runs BOTH triage scenarios against one
 * aimock instance, so the checked-in file is the concatenation of the two
 * scenarios' fixtures (their match keys are disjoint, so one aimock serves both).
 * If the checked-in copy drifts, the demo would replay a different conversation
 * than the scenarios define. This test fails on any such drift; it needs no
 * runtime.
 */
...
describe("aimock fixtures stay in sync with the published scenarios", () => {
  it("static fixtures equal both scenarios' aimockFixtures, concatenated", () => {
    const expected = {
      fixtures: [
        ...(CUSTOMER_DATA_REJECTION_SCENARIO.aimockFixtures as FixtureSet).fixtures,
        ...(MERCHANT_BLOCKED_SCENARIO.aimockFixtures as FixtureSet).fixtures,
      ],
    };
